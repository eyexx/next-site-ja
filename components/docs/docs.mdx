...
この追記より、ファイルシステムがメインのAPIとなります.全ての `.js` という拡張子がつくファイルは 自動的に処理されてリンダリングされるルートとなります。

`./pages/index.js` をあなたのプロジェクト内に作ってください:

```jsx
export default () => <div>Welcome to next.js!</div>
```

`npm run dev`コマンドを動かし、`http://localhost:3000`を見てみましょう。 他のポートを使う場合は,  `npm run dev -- -p <ポート名>`を実行してください。

ここまでで, 以下が可能になりました:

- 自動的なトランスパイレーションとバンドリング (webpackとbabelによる)
- ホットコードリローディング
- サーバーレンダリングと `./pages`のインデクシング
- 静的ファイル提供。 `./static/` は `/static/` に変換される(given you [create a `./static/` directory](#static-file-serving-eg-images) inside your project)

これがどれだけシンプルであるかを知りたいなら,  [sample app - nextgram](https://github.com/zeit/nextgram)このリポジトリをチェックしてみてください。

### 自動コード分割

あなたが宣言した全ての`import`は バンドリングされそれぞれのページに供給されます. つまり各ページは不必要なコードを読み込む必要がないのです！
```jsx
